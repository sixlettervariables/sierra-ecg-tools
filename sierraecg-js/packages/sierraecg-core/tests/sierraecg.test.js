...
test('Ecg.fromXmlAsync', function (p) {
  p.test('can read SierraECG 1.03 file', function (t) {
    ...
    }).catch(err => t.error(err, 'Should not throw an error'))
    .finally(() => t.end());
  });

  p.test('can read SierraECG 1.04 file', function (t) {
    const xml = fs.readFileSync(fixture('3191723_ZZDEMOPTONLY_1-04_orig.xml'));
    const now = new Date().getTime();
    Ecg.fromXmlAsync(xml).then(ecg => {
      t.comment(`parsed in ${new Date().getTime() - now} ms`);
  ...
  p.test('can read SierraECG 1.04.01 file with UTF-16LE BOM', function (t) {
    const xml = fs.readFileSync(fixture('2020-5-18_15-48-11.xml'), 'utf16le');
    const now = new Date().getTime();
    Ecg.fromXmlAsync(xml).then(ecg => {
      t.comment(`parsed in ${new Date().getTime() - now} ms`);
